...
function filterItems(items: readonly ReferenciakReference[], filter: ReferenciakFilterId) {
  ...
  if (filter === 'quoted') {
    return items.filter((item) => Boolean(item.description));
  }
  return items;
}
...
export default function ReferencePortfolio({
  ...
  return (
    <section className="reference-portfolio">
      ...
      <motion.ul
        layout={!reduced}
        className="reference-portfolio__grid"
        aria-live="polite"
      >
        <AnimatePresence mode="popLayout">
          {visibleItems.map((item, index) => (
            ...
            >
              ...
              <div className="reference-portfolio__meta">
                <span className="reference-portfolio__name">{item.name}</span>
                {item.description && (
                  <p className="reference-portfolio__note">Partneri ajánlás</p>
                )}
              </div>
            </motion.li>
          ))}
