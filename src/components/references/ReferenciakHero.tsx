...
import './ReferenciakHero.css';

type ReferenciakHeroProps = {
  label: string;
  title: string;
  intro: string;
  image: string;
...
export default function ReferenciakHero({
  label,
  title,
  intro,
  image,
  imageAlt,
}: ReferenciakHeroProps) {
  const ref = useRef<HTMLElement>(null);
  const inView = useInView(ref, { once: true, margin: '-5%' });
  const reduced = useReducedMotion();

  return (
    <header ref={ref} className="referenciak-hero">
      <div className="referenciak-hero__bg" aria-hidden="true" />
      <div className="content-wrap referenciak-hero__grid">
        <motion.div
          className="referenciak-hero__copy"
        ...
        >
          ...
          <p className="referenciak-hero__intro">{intro}</p>
        </motion.div>

        <motion.figure
          className="referenciak-hero__media"
