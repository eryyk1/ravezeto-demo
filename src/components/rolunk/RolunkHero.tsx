...
import './RolunkHero.css';

type RolunkHeroProps = {
  label: string;
  title: string;
  intro: string;
  image: string;
...
export default function RolunkHero({ label, title, intro, image, imageAlt }: RolunkHeroProps) {
  const ref = useRef<HTMLElement>(null);
  const inView = useInView(ref, { once: true, margin: '-5%' });
  const reduced = useReducedMotion();

  return (
    <header ref={ref} className="rolunk-hero">
      <div className="rolunk-hero__bg" aria-hidden="true" />
      <div className="content-wrap rolunk-hero__grid">
        <motion.div
          className="rolunk-hero__copy"
        ...
        >
          ...
          <p className="rolunk-hero__intro">{intro}</p>
        </motion.div>

        <motion.figure
          className="rolunk-hero__media"
