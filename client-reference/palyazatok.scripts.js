...
const reduceP=matchMedia('(prefers-reduced-motion: reduce)').matches;
if(!reduceP){
  const so=new IntersectionObserver(es=>es.forEach(e=>{
    ...
    const tick=n=>{const p=Math.min((n-st)/1100,1);e.target.textContent=Math.round(t*(1-Math.pow(1-p,3)))+s;if(p<1)requestAnimationFrame(tick)};
    requestAnimationFrame(tick);
  }),{threshold:.5});
  document.querySelectorAll('.fact .big[data-t]').forEach(el=>so.observe(el));
}
